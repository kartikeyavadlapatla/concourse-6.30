...
aws_s3_bucket=tile-storage-bucket-bcbsma
...
delete_backup()
{
        ...
        total_backups=$( /bin/ls -ltr /home/ubuntu/Control_Plane_BBR/control_plane_backups | /bin/grep -v total | /usr/bin/awk '{print $NF}' | /usr/bin/wc -l )
        ...
        if [[ ${total_backups} > ${backups_tobe_retained} ]]; then
                ...
                for old_backup in $( /bin/ls -ltr /home/ubuntu/Control_Plane_BBR/control_plane_backups | /bin/grep -v total | /usr/bin/awk '{print $NF}' | /usr/bin/head -${backups_tobe_deleted})
                do
                        echo "$(/bin/date) removing /home/ubuntu/Control_Plane_BBR/control_plane_backups/${old_backup}"
                        /bin/rm -Rf /home/ubuntu/Control_Plane_BBR/control_plane_backups/${old_backup}
                done
        else
...
om -e env export-installation --output-file ${backup_dir}/installation_${timestamp}
...
/usr/bin/bbr director --private-key-path private_key.pem --username bbr --host 10.0.1.21 --debug pre-backup-check > ${log_director}
...
bbr director --private-key-path private_key.pem --username bbr --host 10.0.1.21 --debug backup --artifact-path ${backup_dir} >> ${log_director} 2>&1
...
if [[ $? != 0 ]]; then
        ...
        files=$(/bin/ls ${backup_dir}/10.0.1.21*)
        IFS=$'\n'$'\r'
        PATH=$(/bin/ls ${backup_dir}/ | /bin/grep 10.0.1.21)
        for j in ${files}
        do
           /usr/local/bin/aws s3 cp ${backup_dir}/10.0.1.21_*/${j} s3://${aws_s3_bucket}/${backup_dir}/${PATH}/${j}
        done
...
export BOSH_CLIENT=ops_manager BOSH_CLIENT_SECRET=mq_xW-JKPxcQ8A3xWUkbrjg9Yvt7fWA2 BOSH_CA_CERT=~/root.cert BOSH_ENVIRONMENT=10.0.1.21

#bosh -d concourse manifest > $backup_dir/deployed-manifest-$timestamp.yml

/usr/bin/bbr deployment --target 10.0.1.21 --deployment concourse --username bbr_client --password l76J6xyVh5DiD4jPWTBmeswwgxg-WEGX --ca-cert ~/root.cert --debug pre-backup-check > ${log_concourse} 2>&1
...
/usr/bin/bbr deployment --target 10.0.1.21 --deployment concourse --username bbr_client --password l76J6xyVh5DiD4jPWTBmeswwgxg-WEGX --ca-cert ~/root.cert --debug backup --with-manifest --artifact-path ${backup_dir} >> ${log_concourse} 2>&1
/bin/chmod -R 777 ${backup_dir}
if [[ $? != 0 ]];then
...
/usr/bin/bbr deployment --target 10.0.1.21 --deployment concourse --username bbr_client --password l76J6xyVh5DiD4jPWTBmeswwgxg-WEGX --ca-cert ~/root.cert --debug backup-cleanup >> ${log_concourse} 2>&1
